...
## Project Overview
KVGenius is a Flet-based desktop AI studio for image generation and party-game card writing. Image generation runs against a local **ComfyUI** server; the Card Generator writes text via a local **Ollama** server. Neither model backend is loaded in-process by KVGenius itself — the app is an HTTP client to both services. (Chat, with AI characters/personas and conversation memory, moved to a separate app.)
...
- **Ollama** must be running locally (default `http://localhost:11434`) for the Card Generator tab to work.
...
- `core/` — UI-agnostic backend: `chat_gen.py` (Ollama client wrapper, used only by Card Generator), `image_gen.py` (ComfyUI client wrapper), `ollama_client.py`, `comfyui_client.py`, `config.py`.
...
- `src/training/lora_trainer.py` — image LoRA training pipeline (local `torch`/`diffusers`/`peft`, still in-process — this is the one place those heavy ML packages are still required; not in `requirements.txt` by default).
- `src/cards/` — Cards Against Humanity-style card generator logic.
- `config/*.yaml` — model presets, settings, prompt templates.
- `fix_dll_paths.py` — **CRITICAL**: Must be imported before `torch` in any script that imports `torch` (only relevant to local LoRA training).
- `scripts/` — utility scripts (model/LoRA downloads, image conversion, PyTorch build helper).

## Development Guidelines
- Image generation and card text generation go through `core/ollama_client.py` / `core/comfyui_client.py` — do not add new in-process `transformers`/`diffusers` model loading for those two features.
...
- **Always import `fix_dll_paths` before importing `torch`** in any script/module that touches `torch` (LoRA training only).

## Hardware Setup
- **GPU:** NVIDIA GeForce RTX 5070 Ti (16 GB VRAM)
- **Compute Capability:** sm_120 (Blackwell architecture)
- **PyTorch:** Custom-built from source (see `scripts/build_pytorch_sm120.bat`) — only needed for local LoRA training, not for image generation or card writing.
- **CUDA:** v13.0+
...
**Solution:** Expected unless you've separately installed `torch`/`diffusers`/`peft` for local LoRA training — image generation and card writing don't need them.
